...
# We need a transition function that's not provided in the package, so we define
# it here.
...
winter_tick <- config(
  life_cycle(
    ...
    transition(
      ...
      expo_shifted_fun,
      'probability',
      predictors = list(x = predictor_spec('max_temp')),
      parameters = list(a = 0.01, b = 1.2, c = 15)
    ),
    transition(
      'e_a',
      NULL,
      ...
      predictors = list(x = predictor_spec('snow_cover')),
      mortality_type = 'per_day',
      parameters = c(no_snow_mort = 0.11, snow_mort = 0.64)
    ),
...
TODO left off here: investigate why the population declines so rapidly/is only above 1 individuals at the beginning. May be helpful to try to checkout the code when `dave_script.R` was first written/run, and see whether the result was same then. If not, that could indicate that something got messed up with the config (or the `run()` code.
