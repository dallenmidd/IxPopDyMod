...
We start with `config_ex_1`, a simple model configuration that doesn't consider infection, and that has four life stages: `egg`, `larva`, `nymph`, and `adult`. This `config` is already loaded with the package, but as an example here is how the `config` is specified in R.
